...
public class SignUpServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(userService.getUserByEmail(signUpData.getEmail()).isPresent() || signUpData.getEmail().length() > 30
        || signUpData.getName().length() > 30 || signUpData.getPassword().length() > 1000) {
            req.setAttribute("emailIsPresent", true);
            req.getRequestDispatcher(SIGN_UP_PAGE).forward(req, resp);
        }
